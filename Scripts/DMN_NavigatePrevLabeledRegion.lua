...
--[[
    DMN_NavigatePrevLabeledRegion

    Moves the playhead to the previous region that is "owned" by the active label.
...
local function main()
    ...
    if current_idx then
        -- Step back one from the current region
        if current_idx > 1 then
            target = labeled[current_idx - 1]
        end
    else
        -- Not inside any labeled region — jump to the nearest one before the cursor
        for i = #labeled, 1, -1 do
            if labeled[i].start < cursor_pos then
                target = labeled[i]
                break
            end
        end
        -- If nothing found before, wrap to the last one
        if not target then
